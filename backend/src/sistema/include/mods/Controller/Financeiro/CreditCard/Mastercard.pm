...
# 2008
package Controller::Financeiro::CreditCard::Mastercard;
...
sub visa_tipo {
	...
	return '4001' if $_[0] eq 'S' || $_[1] < 2; #1001 A Vista
	return '5' if $_[0] eq 'L';	#2xxx Parcelado Loja 2x+
	return '6' if $_[0] eq 'O';	#3xxx Parcelado Administradora 2x+
...
sub visa_authorize { #Tipo Moset, pagamento recorrente, não é vbv
	...
	if ($self->invoice('nossonumero')) { #testar para saber se foi aprovada
		...
		#return %result if $result{'ARS'} eq $self->lang(2118); #Capturadas
	}
	...
	my $exp = substr($self->creditcard('exp'),2,2).substr($self->creditcard('exp'),0,2); #VisaNet é invertido: aamm
