...
# 2011
package Controller::ControlPanel::WebSpider::Enkompass2;
...
sub baseurl {
	my ($self) = shift;
	return $self->{_enkom}{$self->srvid}{baseurl} if $self->{_enkom}{$self->srvid}{baseurl};
	
	my $protocol = $self->server('ssl') ? 'https' : 'http';
	my $port = $self->server('port') || 80; 
	$self->{_enkom}{$self->srvid}{baseurl} = sprintf('%s://%s:%d',$protocol,$self->server('host'),$port);
	
	return $self->{_enkom}{$self->srvid}{baseurl};
}
...
sub login {
	...
	$self->{_enkom} = undef; #reset
	my $url = $self->baseurl;
	$self->get($url);
	#logon
	$self->form_name('form1');
	$self->field('Login1$UserName',$self->server('username'));
	$self->field('Login1$Password',$self->server('key'));
	$self->field('__EVENTTARGET','Login1$LoginButton');
	$self->field('__EVENTARGUMENT','');	
	$self->click('Login');
	die $self->content();	
...
sub quicksearch {#verifica se já existe
	...
	unless ($self->success and @matches = $self->content =~ m/No Items Found/gsi) {
		if ($_[0] eq 'CUSTOMERS') {
			...
			$self->{_enkom}{$self->srvid}{customers}{lc $quest} = $id if $id;
		} else {
			my ($id) = $self->content =~ m/\<a href="\/Interface\/Domains\/Domain\.aspx\?DomainId=(\d+?)\&amp\;AccountId=(\d+?)"\>$quest\<\/a\>/is;
			$self->{_enkom}{$self->srvid}{domains}{lc $quest} = $id if $id;
		}
...
sub list_customers {
	...
	my $url = $self->baseurl . '/Interface/Customers/Customers.aspx?AccountId='.$self->{_enkom}{$self->srvid}{id};
	...
	if ($self->success) {
		...
		%{$self->{_enkom}{$self->srvid}{customers}} = reverse @pairs;
	} else {
...
sub chooseuser {
    ...
    $self->list_customers;
    my $i;
	while ($self->{_enkom}{$self->srvid}{customers}{$user}) {
...
sub search_role {
	...
	my $url = $self->baseurl . '/Interface/AccountRoles/AccountRoles.aspx?AccountId='.$self->{_enkom}{$self->srvid}{id};
	$self->get($url);
...
sub create_role {
	...
	my $url = $self->baseurl . '/Interface/AccountRoles/AccountRole.aspx?AccountId='.$self->{_enkom}{$self->srvid}{id};
	$self->get($url);
...
sub create_customer {
	...
	my $url = $self->baseurl . '/Interface/Customers/CustomerAdd.aspx?AccountId='.$self->{_enkom}{$self->srvid}{id};
	$self->get($url);
...
sub create_package {
	...
	$self->quicksearch('CUSTOMERS',1) unless $self->{_enkom}{$self->srvid}{customers}{$self->service('useracc')};
	my $url = $self->baseurl . '/Interface/Packages/AddPackage.aspx?AccountId='.$self->{_enkom}{$self->srvid}{customers}{$self->service('useracc')};;
	$self->get($url);
...
sub create_domain {
	...
	$self->quicksearch('CUSTOMERS',1) unless $self->{_enkom}{$self->srvid}{customers}{$self->service('useracc')};
	my $url = $self->baseurl . '/Interface/Domains/DomainCreate.aspx?AccountId='.$self->{_enkom}{$self->srvid}{customers}{$self->service('useracc')};;
	$self->get($url);
...
sub nameserver_mask {
	...
	$self->quicksearch('CUSTOMERS',1) unless $self->{_enkom}{$self->srvid}{customers}{$self->service('useracc')};
	my $url = $self->baseurl . '/Interface/Account/AccountSettings.aspx?AccountId='.$self->{_enkom}{$self->srvid}{customers}{$self->service('useracc')};
	$self->get($url);
...
sub create_nameserver {
	...
	my $url = $self->baseurl . '/Interface/Servers/Servers.aspx?AccountId='.$self->{_enkom}{$self->srvid}{id};
	$url .= '&__EVENTTARGET=_ctl3$grid$Pager&__EVENTARGUMENT=Page,-1'; #show all
	$self->get($url);
...
sub chosting {
	...
	$self->quicksearch('CUSTOMERS',1);
	$self->{_enkom}{$self->srvid}{id} = $self->{_enkom}{$self->srvid}{customers}{lc $self->setting('user_host_win')};
...
sub remove_customer {
	...
	$self->quicksearch('CUSTOMERS',1) unless $self->{_enkom}{$self->srvid}{customers}{$self->service('useracc')};
	my $url = $self->baseurl . '/Interface/default.aspx?AccountId='.$self->{_enkom}{$self->srvid}{customers}{$self->service('useracc')};
	$self->get($url);
