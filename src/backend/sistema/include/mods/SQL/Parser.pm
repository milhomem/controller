...
#substituir column_defs por column_names
#comentei os SQL STYLE comments, pois pegava -- dentro do comando de verdade
######################################################################
package SQL::Parser;
######################################################################
#
...
sub parse
{
    ...
    if ( $self->{opts}->{valid_commands}->{$com} )
    {
        ...
        if (    defined( $self->{struct}->{column_names} )
             && defined( _ARRAY( $self->{struct}->{column_names} ) ) )
        {
            foreach my $col ( @{ $self->{struct}->{column_names} } )
            {
                next
...
sub SET_CLAUSE_LIST
{
    ...
    return undef
      unless ( $self->{struct}->{column_names} = $self->ROW_VALUE_LIST( join ',', @cols ) );
